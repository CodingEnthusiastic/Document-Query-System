...
class EntityExtraction:
    ...
    def download_spacy(self, spacy_type):
        """Download or load spacy model

        :param spacy_type: e.g., "en_core_web_sm"
        :type spacy_type: string
        """
        logging.info(f'Loading spaCy model: {spacy_type}')

        try:
            self.nlp = spacy.load(spacy_type)
        except OSError:
            from spacy.cli import download
            logging.info(f"Downloading spaCy model: {spacy_type}")
            download(spacy_type)
            self.nlp = spacy.load(spacy_type)
